...
For the Red Hat Tech Exchange events we're utilising Ravello to provide the resources required for this lab, and whilst it may not be the most performant solution it's great for giving us on-demand, scalable, and flexible lab sessions. It's important to note that all of Ravello is virtualised, therefore all of the "baremetal" nodes you'll leverage are actually virtual machines, and we have to mimic IPMI (baremetal control) capabilities through the Ravello API, although all of this will have been pre-configured for you, so the process you follow in this lab will be no different to what you'd do with real baremetal. Ultimately, the aim of this lab is to make it "as close as feasibly possible" to a real-world scenario, albeit within the constraints of the Ravello platform.

Each dedicated environment has a '**jumphost**' or '**workstation**' that acts as a gateway in, and is uniquely identified through DNS. Once you've connected to your workstation virtual machine below, you'll be provided with instructions on how to complete the lab. Everyone's instructions are identical after connection to the jumphost, as everyones environment has been provisioned from the same template.
